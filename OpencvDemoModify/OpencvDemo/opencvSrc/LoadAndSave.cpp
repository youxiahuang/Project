...
static ImageEncoder findEncoder( const String& _ext )
{
	...
	*/
		...
		imread_(const String& filename, int flags, int hdrtype, Mat* mat = 0)
	{
		...
		if (decoder->setScale(scale_denom) > 1) // if decoder is JpegDecoder then decoder->setScale always returns 1
		{
			...
			//ImgProc::resize(*mat, *mat, Size(size.width / scale_denom, size.height / scale_denom));
		}
	...
	*/
	//inline Mat imread(const String& filename,int flags = IMREAD_COLOR)
...
CV_IMPL int cvHaveImageWriter(const char* filename)
{
	...
	ImageEncoder encoder = cv::findEncoder(filename);
...
cvLoadImage(const char* filename, int iscolor)
{
	...
	//return (IplImage*)cv::imread_(filename, iscolor, LOAD_IMAGE);
}
...
cvLoadImageM(const char* filename, int iscolor)
{
	...
	//return (CvMat*)cv::imread_(filename, iscolor, LOAD_CVMAT);
}
