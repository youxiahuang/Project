...
icvCreateHidHaarClassifierCascade( CvHaarClassifierCascade* cascade )
{
 ...
 //       CV_Error( CV_StsNullPtr, "" );
	...
	if (cascade->hid_cascade){
		printf("CV_StsError,hid_cascade has been already created\n");
	}
	if (!cascade->stage_classifier){
		printf("CV_StsNullPtr\n");
	}
	if (cascade->count <= 0){
		printf("CV_StsOutOfRange,Negative number of cascade stages\n");
	}
...
CvSeq*
cvHaarDetectObjectsForROC_(const CvArr* _img,
                     ...
                     CvSize minSize, CvSize maxSize, bool outputRejectLevels )
{
	...
	Ptr<CvMat> temp, sum, tilted, sqsum, normImg, sumcanny, imgSmall;
    CvSeq* result_seq = 0;
    ...
    result_seq = cvCreateSeq( 0, sizeof(CvSeq), sizeof(faceDetect::CvAvgComp), storage );

    if( CV_MAT_CN(img->type) > 1 )
    {
		cvCvtColor(img, temp, COLOR_BGR2GRAY);
        //ImgProc::cvCvtColor(img, temp, COLOR_BGR2GRAY);
        img = temp;
    }
    ...
    if( flags & CV_HAAR_SCALE_IMAGE )
    {
        ...
        for( factor = 1; ; factor *= scaleFactor )
        {
			...
			cvResize(img, &img1, INTER_LINEAR);
            cvIntegral( &img1, &sum1, &sqsum1, _tilted );
			//ImgProc::cvResize(img, &img1, INTER_LINEAR);
			//ImgProc::cvIntegral(&img1, &sum1, &sqsum1, _tilted);
			...
			Mat _norm1 = cvarrToMat(&norm1), _mask1 = cvarrToMat(&mask1);
			parallel_for_(Range(0, stripCount),
				faceDetect::HaarDetectObjects_ScaleImage_Invoker(cascade,
				(((sz1.height + stripCount - 1) / stripCount + ystep - 1) / ystep)*ystep,
				factor, cvarrToMat(&sum1), cvarrToMat(&sqsum1), &_norm1, &_mask1,
				Rect(equRect), allCandidates, rejectLevels, levelWeights, outputRejectLevels, &mtx));
		}
    }
    else
    {
		...
		//ImgProc::cvIntegral(img, sum, sqsum, tilted);
        ...
        for( ; n_factors-- > 0; factor *= scaleFactor )
        {
			...
			parallel_for_(Range(startY, endY),
				faceDetect::HaarDetectObjects_ScaleCascade_Invoker(cascade, winSize, Range(startX, endX),
				ystep, sum->step, (const int**)p,
				(const int**)pq, allCandidates, &mtx));
            ...
            if( findBiggestObject && !allCandidates.empty() && scanROI.area() == 0 )
            {
				...
				faceDetect::groupRectangles(rectList, std::max(minNeighbors, 1), GROUP_EPS);
    ...
    if( minNeighbors != 0 || findBiggestObject )
    {
        if( outputRejectLevels )
        {
			...
			faceDetect::groupRectangles(rectList, rejectLevels, levelWeights, minNeighbors, GROUP_EPS);
        }
        else
        {
			//groupRectangles(rectList, rweights, std::max(minNeighbors, 1), GROUP_EPS);
			faceDetect::groupRectangles(rectList, rweights, std::max(minNeighbors, 1), GROUP_EPS);
        }
    }
    else
    ...
    {
        faceDetect::CvAvgComp result_comp = {CvRect(),0};
        ...
        for( size_t i = 0; i < rectList.size(); i++ )
        {
			...
			if (r.area() > Rect(result_comp.rect).area())
            {
                result_comp.rect = r;
                     ...
                     int minNeighbors, int flags, CvSize minSize, CvSize maxSize )
{
	...
	return cvHaarDetectObjectsForROC_(_img, cascade, storage, fakeLevels, fakeWeights,
