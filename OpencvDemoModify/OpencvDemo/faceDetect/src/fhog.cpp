...
cv::Mat fhog::fhog( const cv::Mat_<float> &mximage, const int sbin)
{
	...
	float *im = new float[dims[0] * dims[1] * 3];
	...
	else
	{
		...
		//mxfeat.create(out[2], out[0] * out[1], CV_64FC1);
		mxfeat.create(out[2], out[0] * out[1], CV_32FC1);
		assert (!mxfeat.empty());
		//cout << "POS" << endl;
  ...
  //          1, 1, 1, 1, cv::BORDER_REPLICATE);
        ...
        for (int ch = 0; ch < out[2]; ch++) {
			for (int col = 0; col < out[1]; col++) {
				for (int row = 0; row < out[0]; row++) {
					mxfeat.at<float>(ch, row*out[1]+col) =  feat[count];
					count++;
				}
			}
		}
